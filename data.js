fetch("./data.json")
  ...
  .then((json) => {
    //searchresults.js
    const searchTerm = () => {
      const unfilteredURL = new URL(document.location).searchParams.get(
        "search"
      );
    ...
    const appendHeader = (filter, id) => {
      const process = document
        .getElementById(id)
        .appendChild(createHeader(filter));
      return process;
    };
